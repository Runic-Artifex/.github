...
export function verifyCompatibilitySet(candidate, schema, releaseManifest) {
  ...
  for (const name of ["dotnetSdk", "node", "bun", "npm", "pnpm"]) if (!exactToolPattern.test(toolchain[name] ?? "")) errors.push(`$.toolchain.${name}: must be an exact numeric version`);
  ...
  for (const source of sources.values()) {
    ...
    if (manifestPath) {
      try {
        ...
        const packageManagers = new Set([`bun@${candidate.toolchain.bun}`, `npm@${candidate.toolchain.npm}`, `pnpm@${candidate.toolchain.pnpm}`]);
        if (!packageManagers.has(manifest.packageManager)) errors.push(`${source.repository}: ${manifestPath} packageManager '${manifest.packageManager ?? "missing"}' disagrees with compatibility Bun '${candidate.toolchain.bun}', npm '${candidate.toolchain.npm}', and pnpm '${candidate.toolchain.pnpm}'`);
